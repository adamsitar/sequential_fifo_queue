...
template <size_t block_size, homogenous upstream_t, typename tag = void>
  requires is_power_of_two<block_size>
class unique_dynamic_buffer : public std::pmr::memory_resource {
  static constexpr size_t blocks_per_segment =
      upstream_t::block_size_v / block_size;

  static_assert(upstream_t::block_size_v >= block_size,
                "Upstream block size must be >= requested block size");
  static_assert(
      upstream_t::block_size_v % block_size == 0,
  ...
  using freelist_type = freelist_storage<block_size, blocks_per_segment>;
...
private:
  struct segment_metadata {
    ...
    result<block_type *> try_allocate() noexcept {
      ...
      return TRY(freelist->pop(freelist_head, freelist_count));
    }

    result<> deallocate(block_type *block, upstream_t *upstream) noexcept {
      auto *freelist =
          reinterpret_cast<freelist_type *>(static_cast<void *>(segment_ptr));
      TRY(freelist->push(*block, freelist_head, freelist_count));

      if (is_empty()) {
        TRY(upstream->deallocate_block(segment_ptr));
        segment_ptr = nullptr;
        return {};
      }
      return {};
  ...
  static constexpr size_t max_segment_count{upstream_t::block_size_v /
...
public:
  ...
  static constexpr size_t block_count_v =
      blocks_per_segment * max_segment_count; // total max capacity
  static constexpr size_t total_size_v = block_size * block_count_v;
  ...
  result<size_t> find_segment_for_pointer(std::byte *ptr) const noexcept {
    ...
    // Try lookup cache (temporal locality - recently looked up blocks)
  ...
  result<pointer_type> allocate_block() noexcept {
    if (_alloc_cache < _high_water_mark) {
      ...
      if (block_result) {
        return pointer_type(_alloc_cache, *block_result);
      }
    }
  ...
  result<> deallocate_block(pointer_type ptr) {
    ...
    TRY(metadata.deallocate(block, _upstream));
    return {};
  };
...
private:
  ...
  result<pointer_type> allocate_new_segment() noexcept {
    size_t slot = TRY(find_free_slot());

    auto upstream_block = TRY(_upstream->allocate_block());
    ...
    freelist->reset(_segments[slot].freelist_head,
                    _segments[slot].freelist_count);

    _segments[slot].segment_ptr = upstream_ptr;
    _alloc_cache = slot;

    if (slot >= _high_water_mark) {
      _high_water_mark = slot + 1;
    }
  ...
  void *do_allocate(size_t bytes, size_t alignment) override {
    if (bytes > block_size || alignment > block_size) {
      return nullptr;
    }
